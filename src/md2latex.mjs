...
import ts2latex from "ts2latex";
import tex from "./latexStrings.json";
...
export default (data, latexCb, jsonCb = () => {}) => {
  ...
  for (let section of sections) {
    ...
    let marker = sectionTitle.replace(/ /g, "").toLowerCase();
  ...
  const readElements = search => {
    ...
    let structure = {
      subsection: seek => {
        let marker = seek[1].replace(/ /g, "").toLowerCase();
      ...
      algorithmBlock: seek => {
        let marker = seek[1].match(regex.algorithmMarker)[1];
        obj.markers.algorithm.push(marker);
        ts2latex(seek[1], (err, latex) => {
          if (err) throw err;
          elements.push({
            type: "algorithmBlock",
            index: seek.index,
            algorithmContents: latex + "\n"
          });
        });
      },
      table: seek => {
  ...
  const writeElements = elements => {
    ...
    let structure = {
      ...
      unorderedList: element => {
        ...
        for (let entry of element.entries) {
      ...
      table: element => {
        ...
        for (let row of element.tableContents) {
      ...
      algorithmBlock: element => {
        out += element.algorithmContents;
      }
    };
  ...
  for (let section of obj.sections) {
    ...
    out += tex.sectionLabelBegin + section.sectionMarker + tex.sectionLabelEnd;
